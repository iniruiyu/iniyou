...
- Flutter 设置入口现已补齐自定义主题工作台：除固定主题外，还支持与 Legacy Web 对齐的 `custom` 主题、4 套精选预设，以及 `bg / surface / surfaceSoft / primary / primaryStrong / accent / text / muted` 八个令牌的即时编辑与本地持久化。 The Flutter settings entry now includes a full custom-theme workbench aligned with the Legacy Web frontend: besides fixed themes it supports a `custom` skin, four curated presets, and live local editing/persistence for the eight `bg / surface / surfaceSoft / primary / primaryStrong / accent / text / muted` tokens.
