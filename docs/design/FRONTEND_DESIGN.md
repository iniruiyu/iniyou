...
- Flutter 宽屏导航按钮需统一为主题感知的半透明玻璃态组件，顶部折叠按钮、顶部导航 chip 与侧栏导航按钮保持同一套圆角、描边与高亮逻辑 / Flutter wide-layout navigation buttons should use theme-aware translucent glass components consistently, with the collapsed top trigger, top-nav chips, and sidebar buttons sharing the same corner radius, border, and highlight logic.
...
- 2026-03-27：可选微服务探测改成短超时，个人主页里对空间服务的拉取在离线时直接跳过，避免打开好友资料或服务导航时卡在等待上 / Optional microservice probes now use short timeouts, and profile-space loads skip immediately when offline so opening friend profiles or the service navigator no longer stalls.
...
- 2026-03-27：继续将 Vue 服务导航页拆成独立组件，减少 `app.js` 根模板膨胀，并给后续页面拆分留出统一样板 / Continued splitting the Vue service navigation page into a standalone component to keep the root template lean and provide a reusable pattern for later page splits.
- 2026-03-27：继续把 Vue 个人主页拆成独立组件，让根模板只负责壳层和挂载收口，方便后续继续拆身份编辑与资料卡 / Continued splitting the Vue personal home into a standalone component so the root template only handles the shell and mount point, leaving room for later identity-editor and card splits.
- 2026-03-27：继续把个人资料编辑器和会员切换浮层也拆成独立组件，让个人主页保持纯展示职责 / Continued splitting the profile editor and membership sheet into standalone components so the personal home can stay display-only.
