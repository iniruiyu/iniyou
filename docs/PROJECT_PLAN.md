...
- 2026-03-26：把 Flutter 和 Vue 聊天页“最近会话”入口都收纳到右侧菜单，紧凑布局不再让左侧会话列表常驻占位 / Folded both Flutter and Vue chat page "Recent conversations" entries into a right-side menu so compact layouts no longer reserve the left conversation list permanently.
- 2026-03-26：把 Vue 登录后账号、会员、语言、设置与退出入口收进右上角，顶栏不再把这组操作挤到页面中部 / Moved the Vue post-login account, membership, language, settings, and sign-out actions into the upper-right corner so the top bar no longer drifts these controls toward the page center.
- 2026-03-26：把 Flutter 登录后账号、刷新、设置与退出入口收进右上角，并用紧凑账号块避免长用户名挤散顶栏 / Moved the Flutter post-login account, refresh, settings, and sign-out actions into the upper-right corner and used a compact account chip to keep long usernames from pushing the top bar apart.
- 2026-03-26：把 Flutter 和 Vue 的登录后账号、会员、语言、设置与退出入口统一并入主菜单侧栏，主导航默认折叠并用浮动按钮唤出，让聊天区域获得更多横向空间 / Merged the Flutter and Vue post-login account, membership, language, settings, and sign-out actions into the main menu sidebar, kept the main navigation collapsed by default with a floating toggle, and gave the chat area more horizontal room.
- 2026-03-26：把 Vue 设置面板里的新增语言收成默认折叠块，并将退出登录并入设置面板；Flutter 则恢复非空间页的顶部主导航可见 / Folded Vue's add-language form into a closed-by-default section, moved sign-out into the settings panel, and restored the Flutter top navigation outside the space page.
- 2026-03-26：把 Vue 折叠侧栏改成真正的单列全宽壳层，侧栏收起后主内容区不再保留两栏占位 / Turned the Vue collapsed sidebar into a true single-column full-width shell so the main content no longer keeps two-column space when the sidebar is hidden.
- 2026-03-26：修正 Vue 折叠态主内容区的 grid-column 锚点，避免折叠后仍落在第二列而左侧留白 / Fixed the Vue collapsed-state main-content grid-column anchor so it no longer lands in the second column and leaves blank space on the left.
- 2026-03-26：把 Vue 折叠态壳层直接切成块级布局，避免 grid 轨道继续保留侧栏占位，让主内容真正全宽铺开 / Switched the Vue collapsed shell to block layout so grid tracks stop reserving sidebar space and the main content can truly span full width.
- 2026-03-26：将 Vue 折叠态改回零宽侧栏轨道并直接隐藏 sidebar，确保主内容从视口边缘开始铺满 / Changed the Vue collapsed state back to a zero-width sidebar track and hid the sidebar outright so the main content starts at the viewport edge and fills the screen.
- 2026-03-26：把 Vue 折叠态重构为顶层精简导航，并让展开态侧栏改成浮动左侧抽屉，同时取消主内容对侧栏的空间占位 / Reworked the Vue collapsed state into a top-level compact navigation, turned the expanded sidebar into a floating left drawer, and removed main-content space reservation for the sidebar.
- 2026-03-26：把 Vue 展开态顶部栏彻底隐藏，只在折叠态保留顶层精简导航，避免顶部再占用垂直空间 / Fully hid the Vue top bar in expanded state and kept the compact top-level navigation only when collapsed so the top area no longer consumes vertical space.
...
- 2026-03-26：修正 Flutter 折叠侧栏的展开入口，让顶栏左侧始终可见，并清掉误放在不可达分支里的浮动按钮 / Fixed the Flutter collapsed-sidebar expand trigger so it stays visible in the top bar and removed the floating button that had been placed in an unreachable branch.
- 2026-03-26：把 Flutter 宽屏导航收成单一入口，侧栏展开时隐藏顶部导航、侧栏折叠时显示顶部导航，并让顶栏按钮跟随当前主题色 / Consolidated Flutter wide-screen navigation into a single entry point by hiding the top navigation when the sidebar is expanded, showing it when the sidebar is collapsed, and making the top buttons follow the active theme colors.
- 2026-03-26：账号资料模型新增头像地址与出生日期，Vue/Flutter 个人主页与资料编辑器统一改为展示头像，并根据出生日期派生生日与年龄 / Added avatar-URL and birth-date fields to the account profile model, and updated the Vue/Flutter profile pages plus editors to show avatars and derive birthday/age from the birth date.
