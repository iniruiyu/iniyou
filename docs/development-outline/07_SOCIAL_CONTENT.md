...
- 已完成：
  ...
  - 学习课程 Markdown 文件已拆分到独立 `learning-service`，支持按路径保存和读取 `.md` 文件
...
- 双前端个人主页已收敛为摘要页，本人视图只展示个人资料、联系方式、隐私设置和会员等级，公开空间入口只保留在好友资料里，不再展示内容流 / Both frontends' profile pages are now summary pages: the owner view only shows personal info, contact details, privacy settings, and membership level; public-space entrances stay in friend profiles only, with no content feed.
...
- 2026-03-27：个人主页的公开空间入口改为仅在好友资料里展示，本人视图不再显示自己的空间数量或空间卡片 / Public-space entrances on profile pages are now shown only in friend profiles, and the owner view no longer shows its own space count or space cards.
